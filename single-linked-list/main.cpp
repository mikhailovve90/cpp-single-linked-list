...
class SingleLinkedList {
    ...
    public:
        // Объявленные ниже типы сообщают стандартной библиотеке о свойствах этого итератора(для того что бы итератор работал с
        // всеми методами стандартных библиотек
        ...
        BasicIterator& operator++() noexcept {
            if(this->node_ == nullptr) return *this;
...
public:
    ...
    [[nodiscard]] Iterator end() noexcept {
        if(this->IsEmpty() ) return begin();
        Iterator result(head_.next_node);

        while ( result.node_  != nullptr) {
            ++result;
        }
        return result;
    }
    ...
    [[nodiscard]] ConstIterator before_begin() const noexcept {
        //вот эти две строки в сумме выводил часа три, просто сгорела жопа, от невозможности создания константного указателя на неконстантный объект,
        //без консткаста, а конст каст оказывается кастует конст в неконст если объект переданый в него конст. супер))) такое хорошо бы в подсказку внести.
        Node* const ptr = const_cast<Node*>(&head_);
    ...
    Iterator InsertAfter(ConstIterator pos, const Type& value) {
        ...
        ++size_;
        return Iterator(i_a_node);
    }

    void PopFront() noexcept {
        if(size_ > 0){
            Iterator cbeg = before_begin();
            Iterator beg = begin();
            Node *next_node = beg.node_->next_node;
            delete cbeg.node_->next_node;
            cbeg.node_->next_node = next_node;
            --size_;
        }
    ...
    Iterator EraseAfter(ConstIterator pos) noexcept {
        ...
        if(size_ > 0){
            if(n_n  != nullptr){
                ...
                Node *n_n_n = n_n->next_node;
                delete n_n;
                pos.node_->next_node = n_n_n;
            }
            --size_;
        }
        return Iterator(n_n_n);
    }
    ...
    void PushFront(const Type& value) {
        try {head_.next_node = new Node(value, head_.next_node);}
        catch (...) {throw;}
        ++size_;
    }
    ...
    void Clear() noexcept {
        while (head_.next_node!=nullptr) {
            Node* temporary_node = head_.next_node;
